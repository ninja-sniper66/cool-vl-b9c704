...
function buildTaskDiscoverySection(taskText: string, cwd: string): string {
	try {
		...
		if (topFile) {
			...
			const topDir = dirname(topFile).replace(/\\/g, "/");
			if (topDir && topDir !== ".") {
				sections.push(
					`\nNEW FILE PLACEMENT: If the task gives only a bare filename (for example, \`foo.py\`) with no directory, place it under \`${topDir}/\` unless the task explicitly provides a different path.`,
				);
			} else {
				sections.push(
					"\nNEW FILE PLACEMENT: No confident subdirectory was inferred. For a bare filename, do not guess a directory; only place at repo root when other task signals clearly indicate root-level placement.",
				);
			}
		}
...
- For new files: if the task gives a full path with a directory (e.g., \`scripts/foo.py\`), use it exactly. If the task gives only a bare filename with no directory (e.g., \`foo.py\`), follow the NEW FILE PLACEMENT hint in the discovery section. Never guess; only use repo root when the hint or task signals root-level placement.
